...
int main(int argc, char *argv[])
{
    ...
    else
    {
        if (rank == RANK_0)
        {
            ...
            distribuirCargas(rows, nProcesos, desplazamientos, cuentas);

            for (int i = RANK_1; i < nProcesos; i++)
            {
                int rowVecLen = columns * cuentas[i];

                MPI_Send(matriz[desplazamientos[i]], rowVecLen, MPI_INT, i, 10, MPI_COMM_WORLD);
            }
